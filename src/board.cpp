...
void Board::openBook(std::string file){
    book.clear();
    std::ifstream valFile;
    int count=1;
    valFile.open(file.c_str(), std::ios::in | std::ios::binary);
    if (valFile.good()) {
        valFile.seekg(0, std::ios::beg);
        while (!valFile.eof()) {
            uchar temp;
            Book b;
            b.key=0ULL;
            for(int i=0;i<8;i++){
               valFile.read((char*) &temp, sizeof (uchar));
               b.key=(b.key<<8)|temp;
            }
            for(int i=0;i<2;i++){
               valFile.read((char*) &temp, sizeof (uchar));
               b.move=(b.move<<8)|temp;
            }
            for(int i=0;i<2;i++){
               valFile.read((char*) &temp, sizeof (uchar));
               b.weight=(b.weight<<8)|temp;
            }
            for(int i=0;i<4;i++){
               valFile.read((char*) &temp, sizeof (uchar));
               b.learn=(b.learn<<8)|temp;
            }
            book.push_back(b);
        }
    }
...
void Board::getEntry(){
    ...
    for (std::vector<Book>::iterator it = book.begin(); it < book.end(); it++) {
        if (currPos()->key == it->key){
            if(it->move==0)
                continue;
            bookMove[pos++]=it;
        }
    }
    bookMove[pos]=(std::vector<Book>::iterator)(0);
}
...
U64 Board::hash(){
    ...
    for (int rank = 24; rank < 120; rank += 12) {
        for (int file = 2; file < 10; file++) {
            int p=square[rank+file];
            if(p==0)
                continue;
            int s = 8*(rank/12-2)+file-2;
            if (p<8)
                p=p*2-1; 
            else
                p=(p-9)*2;
            key^=RandomPiece[64*p+s];
        }
    }
    if(currPos()->castle&CASTLE_WK) key^=RandomCastle[0];
    if(currPos()->castle&CASTLE_WQ) key^=RandomCastle[1];
    if(currPos()->castle&CASTLE_BK) key^=RandomCastle[2];
    if(currPos()->castle&CASTLE_BQ) key^=RandomCastle[3];
    if(currPos()->enPassant>=0){
        if(colorToMove()){
            Square eSqr=(Square)(A3+currPos()->enPassant);
            if(!OFF_BOARD[eSqr+11] && square[eSqr+11]==BP)
                key^=RandomEnPassant[currPos()->enPassant];
            else if(!OFF_BOARD[eSqr+13] && square[eSqr+13]==BP)
                key^=RandomEnPassant[currPos()->enPassant];
        } else {
            Square eSqr=(Square)(A6+currPos()->enPassant);
            if(!OFF_BOARD[eSqr-11] && square[eSqr-11]==WP)
                key^=RandomEnPassant[currPos()->enPassant];
            else if(!OFF_BOARD[eSqr-13] && square[eSqr-13]==WP)
                key^=RandomEnPassant[currPos()->enPassant];  
        }
    }
    if(!colorToMove())
...
const Position POSITION_EMPTY = {
   0ULL, NO_MOVE, 0, 0, 0, 0
};

const uchar CASTLE_WK=1;
const uchar CASTLE_WQ=2;
const uchar CASTLE_BK=4;
const uchar CASTLE_BQ=8;
...
/*can also handle epd strings, if best move and comments are removed*/
void Board::positionFromFen(std::string fenIn) {
    fen=fenIn;
    startpos=false;
    startFen=fen;
    int i;
    for (i = 0; i < 500; i++) {
        position[i] = POSITION_EMPTY;
    }
    int rank;
    int file;
    for (file = 0; file < 12; file++)
        for (rank = 0; rank < 12; rank++)
            square[file + rank * 12] = NO_PIECE;
    rank = 9;
    file = 2;
    int pos = 0;
    while (fen[pos] != ' ') {
        if (isdigit(fen[pos])) {
            ...
            std::string pieceStr = "XPNBRQKXxpnbrqkx";
            size_t piecePos = pieceStr.find_first_of(fen[pos]);
            if (piecePos == std::string::npos) {
                std::cerr << "Error reading fen <" << fen << "> in pos "
                        << pos << std::endl;
            }
            square[file + rank * 12] = (Piece) piecePos;
            if (fen[pos] == 'K')
                whiteKing = (Square) (file + rank * 12);
            if (fen[pos] == 'k')
                blackKing = (Square) (file + rank * 12);
            file++;
        }
        pos++;
    }
    ...
    if (fen[pos] == 'b')
        ...
        std::cerr << "Error reading fen <"
            << fen << "> in pos " << pos << std::endl;
    pos += 2;
    /*store castle temporarily in local variable
     since we do not know the move count yet*/
    uchar castle = 0;
    while (fen[pos] != ' ') {
        std::string castleStr = "-KQxkxxxq";
        size_t castlePos = castleStr.find_first_of(fen[pos]);
        if (castlePos == std::string::npos)
            std::cerr << "Error reading fen <"
                << fen << "> in pos " << pos << std::endl;
        castle |= castlePos;
        pos++;
    }
    /*store en passant temporarily in local variable
     since we do not know the move count yet*/
    schar enPassant = -1;
    pos++;
    if (fen[pos] != '-') {
        enPassant = fen[pos] - 'a';
        if (enPassant < 0 || enPassant > 7)
            std::cerr << "Error reading fen <"
                << fen << "> in pos " << pos << std::endl;
        pos++;
    }
    pos += 2;
    int halfMove = 0;
    /*if this is an epd string it ends here*/
    while (pos < fen.length() && fen[pos] != ' ') {
        if (!isdigit(fen[pos]))
            std::cerr << "Error reading fen <" << fen << "> in pos " << pos << std::endl;
        halfMove *= 10;
        halfMove += fen[pos] - '0';
        pos++;
    }
    ...
    if (moveCount < 1)
        moveCount = 1;
    currentPosition = (moveCount - 1)*2 + sideToMove;
    startMove=currentPosition;
    position[currentPosition].castle = castle;
    checkCastle();
    position[currentPosition].enPassant = enPassant;
    position[currentPosition].halfMove = (uchar) halfMove;
    position[currentPosition].key=hash();
}

void Board::checkCastle(){
    if (square[A1] != WR)
        position[currentPosition].castle &= ~CASTLE_WQ;
    else if (square[H1] != WR)
        position[currentPosition].castle &= ~CASTLE_WK;
    else if (square[E1]!=WK)
        position[currentPosition].castle &= ~(CASTLE_WK|CASTLE_WQ);
    else if (square[A8]!=BR)
        position[currentPosition].castle &= ~CASTLE_BQ;
    else if (square[H8]!=BR)
        position[currentPosition].castle &= ~CASTLE_BK;
    else if (square[E8]!=BK)
        position[currentPosition].castle &= ~(CASTLE_BK|CASTLE_BQ);
}

std::string Board::fenFromPosition(){
    std::string fen="";
    for(int rank=108;rank>12;rank-=12){
        int empty=0;
        if(rank<108)
                fen+="/";
        for(int file=2;file<10;file++){
            if(square[file+rank]==NO_PIECE){
                empty++;
            } else {
                if(empty>0){
                    std::stringstream ss;
                ...
                fen+=pieceToString(square[file+rank]);     
            }
        }
        if(empty>0){
            std::stringstream ss;
    ...
    fen+=colorToMove() ? " b ":" w ";
    if(currPos()->castle==0) fen+="-";
    if(currPos()->castle&CASTLE_WK) fen+="K";
    if(currPos()->castle&CASTLE_WQ) fen+="Q";
    if(currPos()->castle&CASTLE_BK) fen+="k";
    if(currPos()->castle&CASTLE_BQ) fen+="q";
    fen+=" ";
    if(currPos()->enPassant>=0){
        fen+=(char)(currPos()->enPassant+'a');
        fen+=colorToMove() ? "3":"6";
    } else { fen+="-"; }
    fen+=" ";
    std::stringstream ss;
    ss<<(int)currPos()->halfMove;
    fen+=ss.str();
    fen+=" ";
    std::stringstream ss2;
    ss2<<(1+currentPosition/2);
    fen+=ss2.str();
    return fen;
}

int Board::repetition(){
    int count=0;
    for(int m=currentPosition;m>=0;m--){
        if(currPos()->key==position[m].key)
            count++;
        if(position[m].move.capPiece!=NO_PIECE)
            break;
        if(removeColor(position[m].move.ownPiece)==P)
            break;
    }
    return count;
}
...
void Board::makeMove(Move move) {
    /*colorToMove is update here*/
    currentPosition++;
    position[currentPosition] = position[currentPosition - 1];
    position[currentPosition].enPassant = -1;
    position[currentPosition].halfMove++;
    position[currentPosition].move = move;
    assert(move.promPiece!= NO_PIECE || move.ownPiece!=P || (move.to<A8 && move.to>H1));
    if (square[move.from] == WK)
        whiteKing = move.to;
    if (square[move.from] == BK)
        blackKing = move.to;
    if (move.capPiece != NO_PIECE || removeColor(square[move.from])==P )
        position[currentPosition].halfMove=0;
    square[move.to] = square[move.from];
    square[move.from] = NO_PIECE;
    if (move.promPiece != 0)
        square[move.to] = (Piece) (move.promPiece+(colorToMove() ? 0 : 8));
    if (move.type != 0) {
        if (move.doubleStep())
            position[currentPosition].enPassant = move.to % 12 - 2;
        else if (move.enPassant())
            square[move.to + (colorToMove() ? -12 : 12)] = NO_PIECE;
        else if (move.castleKing()) {
            if (colorToMove() == BLACK_BOOL) {
                square[H1] = NO_PIECE;
                square[F1] = WR;
            } else {
                square[H8] = NO_PIECE;
    ...
    updateCastle(move.from);
    updateCastle(move.to);
    fen=fenFromPosition();
    //std::cout<<"moved "<<fen<<std::endl;
    position[currentPosition].key=hash();
}

void Board::updateCastle(Square square) {
    if (square == A1)
        position[currentPosition].castle &= ~CASTLE_WQ;
    else if (square == H1)
        position[currentPosition].castle &= ~CASTLE_WK;
...
void Board::unmakeMove() {
    ...
    /*colorToMove is update here*/
    Move move = position[currentPosition].move;
    currentPosition--;
    if (square[move.to] == WK)
        whiteKing = move.from;
    if (square[move.to] == BK)
        blackKing = move.from;
    square[move.from] = square[move.to];
    square[move.to] = move.capPiece;
    if (move.promPiece != 0)
        square[move.from] = colorToMove() ? BP : WP;
    if (move.type != 0) {
        if (move.enPassant())
            square[move.to + (colorToMove() ? 12 : -12)] = colorToMove() ? WP : BP;
        else if (move.castleKing()) {
            if (colorToMove() == WHITE_BOOL) {
                square[F1] = NO_PIECE;
                square[H1] = WR;
            } else {
                square[F8] = NO_PIECE;
...
std::string Board::moveToLongNotation(Move move) {
    if (move.castleKing()) 
        return "O-O";
    if (move.castleQueen())
        return "O-O-O";
    std::string m = "";
    const std::string PS = "X NBRQKX";
    if (move.ownPiece > 1) {
        assert(move.ownPiece < 7);
        m += PS[move.ownPiece];
    }
    m += (char) (file(move.from) + 'a');
    m += (char) (rank(move.from) + '1');
    if (move.capPiece != 0 || move.enPassant())
        m += 'x';
    else 
        m += '-';
    m += (char) (file(move.to) + 'a');
    m += (char) (rank(move.to) + '1');
    if (move.promPiece != 0)
        m += PS[move.promPiece];
    if (move.enPassant() != 0)
        m += "e.p.";
...
std::string Board::moveToSan(Move move) {
    if (move.castleKing()) 
        return "O-O";
    if (move.castleQueen())
        return "O-O-O";
    ...
    if(move.id==ID_FILE || move.id==ID_BOTH
            ||(move.ownPiece==P && (move.capPiece!=NO_PIECE|| move.enPassant())))
        m += (char) (file(move.from) + 'a');
    if(move.id==ID_RANK || move.id==ID_BOTH)
        m += (char) (rank(move.from) + '1');
    if (move.capPiece != 0 || move.enPassant()){
        m += 'x';
    }
...
std::string Board::moveToCompleteString(Move move){
    std::string str=pieceToString(square[move.from])+" ";
    str+=squareToString(move.from)+" "+squareToString(move.to)+" ";
    str+=pieceToString(move.capPiece)+" "+pieceToString(move.promPiece);
    if(move.doubleStep())
    str+= " ds";
    if(move.enPassant())
    str+= " ep";
    ...
    if(move.castleQueen())
        str+=(colorToMove() ? " q":" Q");
    makeMove(move);
    MoveGenerator *movegen=new MoveGenerator(this);
...
std::string Board::moveToUci(Move move){
    std::string str=squareToString(move.from)+squareToString(move.to);
    if(move.promPiece!=NO_PIECE) {
        std::string ps="-pnbrqk";
        str+=ps.substr(move.promPiece,1);
    }
    return str;
}
...
void Board::printBoard() {
    for (int rank = 108; rank > 12; rank -= 12) {
        ...
        for (int file = 2; file < 10; file++) {
            ...
            std::cout << ' ' << pStr[square[file + rank]];
        }
        std::cout << std::endl;
    }
    std::cout<<(colorToMove() ? "black move ":"white move ")<<std::endl;
}

std::string Board::moveToString(MoveString type, Move move){
    if(move.from==0)
        return "";
        switch (type){
            case UCI:
            ...
            case LONG:
                return moveToLongNotation(move);
            case COMPLETE:
                return  moveToCompleteString(move);
        }
    };